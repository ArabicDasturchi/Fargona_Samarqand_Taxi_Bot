...
@driver_router.callback_query(F.data.in_(["dir_to_samarqand", "dir_from_samarqand"]))
async def direction_selected(callback: CallbackQuery):
    direction = callback.data.replace("dir_", "")
    text = "📍 <b>Farg'onadan Samarqandga ketmoqchisiz?</b>" if direction == "to_samarqand" else "📍 <b>Samarqanddan Farg'onaga ketmoqchisiz?</b>"
...
async def route_selected(callback: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        if direction == "to_samarqand":
            from_city = region
            to_city = "Samarqand"
            text_result = f"{region} ⇄ Samarqand"
        else:
            from_city = "Samarqand"
            to_city = region
            text_result = f"Samarqand ⇄ {region}"
